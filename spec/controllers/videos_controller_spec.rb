...
describe VideosController do
  ...
  describe 'the create method' do
    ...
    it 'should send back a redirect to the lesson page on successful creation' do
      ...
      post :create, {"video"=> {"url"=>"a_valid_url"}, "lesson_id"=>"1"}
      response.should redirect_to('/lessons/1')
    end

    it 'should redirect to the video upload page with an invalid video url submission' do
      Video.should_receive(:isValidUrl?).with('a_valid_url').and_return(false)
      post :create, {"video"=> {"url"=>"a_valid_url"}, "lesson_id"=>"1"}
